...
// Helper: wraps db.transaction in a Promise
const runTransaction = (database, fn) => {
    return new Promise((resolve, reject) => {
        database.transaction(
            (tx) => fn(tx, resolve, reject),
            (error) => reject(error)
        );
    });
};

// Helper: runs a single SQL statement inside a transaction
...
export const clear = async () => {
    const database = await db;
    await executeSql(database, `DROP TABLE IF EXISTS UserEntity`);
    await createTables().catch(error => {
        console.error('Error creating tables:', error);
    });
};
